...
class Profitloss extends CI_Controller {
    ...
    public function index() {
         if ($this->sessiondata['username'] != NULL && $this->sessiondata['status'] == 'login' && ($this->sessiondata['userrole'] == 'a' && $this->sessiondata['userrole'] != 's' && $this->sessiondata['userrole'] != 'u' || $this->sessiondata['userrole'] == 'r')):
            ...
            $data['purchasedetails'] = $this->profitlossResult(25, $initialdate, $date_to);
            //Sales account details
            $data['salesdetails'] = $this->profitlossResult(26, $initialdate, $date_to);
            //Direct expense details
            $data['directexpensedetails'] = $this->profitlossResult(16, $initialdate, $date_to);
            //Direct income details
            $data['directincomedetails'] = $this->profitlossResult(17, $initialdate, $date_to);
            //Direct expense details
            $data['indirectexpensedetails'] = $this->profitlossResult(20, $initialdate, $date_to);
            //Direct income details
            $data['indirectincomedetails'] = $this->profitlossResult(21, $initialdate, $date_to);
            $data['date_from'] = $date_from;
            $data['date_to'] = $date_to;
            $this->load->view('header', $data);
            $this->load->view('sidebar', $data);
            $this->load->view('profitloss/profit_loss', $data);
